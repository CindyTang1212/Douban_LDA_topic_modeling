...
if __name__ == '__main__':
    ...
    # get_perplexity(n_features=1500, n_max_topics=10, contents=df['content_cutted'].values.astype('U'))

    # 建立主题模型
    n_topics = 5
    n_features = 1500
    df = pd.read_csv('content_cutted.csv', error_bad_lines=False)
    tf, tf_vectorizer, lda, docres = lda_learning(n_features=n_features, n_topics=n_topics,
    ...
    pyLDAvis.save_html(pic, 'result/' + 'lda_pass' + str(n_topics) + '_' + str(n_features) + '.html')
